...
VIDEO_TRIGGERS_FILE = Path("video_triggers.json")


def _load_video_triggers() -> list[dict]:
    """Load video triggers from JSON file."""
    if not VIDEO_TRIGGERS_FILE.exists():
        return []
    try:
        return json.loads(VIDEO_TRIGGERS_FILE.read_text(encoding="utf-8"))
    except Exception:
        return []


def _save_video_triggers(triggers: list[dict]) -> None:
    """Save video triggers to JSON file."""
    VIDEO_TRIGGERS_FILE.write_text(json.dumps(triggers, ensure_ascii=False, indent=2), encoding="utf-8")


class VideoCommandState:
    """Tracks the current video command to send to kiosk clients."""

    def __init__(self):
        self._command: dict | None = None
        self._issued_at: float = 0.0

    def set(self, video_url: str, trigger_id: str) -> None:
        self._command = {"url": video_url, "trigger_id": trigger_id}
        self._issued_at = time.time()

    def get_pending(self, since: float) -> dict | None:
        """Return command if it was issued after `since` timestamp."""
        if self._command and self._issued_at > since:
            return self._command
        return None


video_command_state = VideoCommandState()


class VideoTriggerRequest(BaseModel):
    """Request model for creating/updating a video trigger."""
    trigger_id: str = Field(..., min_length=1, max_length=100)
    label: str = Field(..., min_length=1, max_length=200)
    keywords: list[str] = Field(..., min_items=1)
    video_filename: str = Field(..., min_length=1, max_length=300)


class VideoPlayRequest(BaseModel):
    """Request model for manually triggering a video."""
    trigger_id: str = Field(..., min_length=1, max_length=100)


@app.post("/admin/video-triggers", response_model=AdminResponse)
async def admin_save_video_trigger(request: VideoTriggerRequest) -> AdminResponse:
    """Create or update a video trigger rule."""
    triggers = _load_video_triggers()
    # Update existing or append
    for i, t in enumerate(triggers):
        if t["trigger_id"] == request.trigger_id:
            triggers[i] = request.model_dump()
            _save_video_triggers(triggers)
            return AdminResponse(message=f"تم تحديث قاعدة التشغيل: {request.label}")
    triggers.append(request.model_dump())
    _save_video_triggers(triggers)
    return AdminResponse(message=f"تمت إضافة قاعدة التشغيل: {request.label}")


@app.get("/admin/video-triggers")
async def admin_get_video_triggers() -> JSONResponse:
    """Get all video trigger rules."""
    triggers = _load_video_triggers()
    videos = [f.name for f in VIDEOS_DIR.iterdir() if f.is_file() and f.suffix.lower() in (".mp4", ".webm", ".mov")]
    return JSONResponse(content={"triggers": triggers, "videos": sorted(videos)})


@app.delete("/admin/video-triggers/{trigger_id}", response_model=AdminResponse)
async def admin_delete_video_trigger(trigger_id: str) -> AdminResponse:
    """Delete a video trigger rule."""
    triggers = _load_video_triggers()
    new_triggers = [t for t in triggers if t["trigger_id"] != trigger_id]
    if len(new_triggers) == len(triggers):
        return AdminResponse(status="error", message="القاعدة غير موجودة")
    _save_video_triggers(new_triggers)
    return AdminResponse(message="تم حذف القاعدة")


@app.post("/admin/video-triggers/play", response_model=AdminResponse)
async def admin_play_video(request: VideoPlayRequest) -> AdminResponse:
    """Manually trigger a video to play on all kiosk clients."""
    triggers = _load_video_triggers()
    trigger = next((t for t in triggers if t["trigger_id"] == request.trigger_id), None)
    if not trigger:
        return AdminResponse(status="error", message="القاعدة غير موجودة")
    video_url = f"/static/videos/{trigger['video_filename']}"
    video_command_state.set(video_url, request.trigger_id)
    logger.info(f"Manual video trigger: {request.trigger_id}")
    return AdminResponse(message=f"تم تشغيل الفيديو: {trigger['label']}")


@app.post("/admin/upload-video", response_model=AdminResponse)
async def admin_upload_video(file: UploadFile = File(...)) -> AdminResponse:
    """Upload a video file for use in triggers."""
    allowed_ext = (".mp4", ".webm", ".mov")
    suffix = Path(file.filename).suffix.lower()
    if suffix not in allowed_ext:
        return AdminResponse(status="error", message="الصيغة غير مدعومة. المسموح به: mp4, webm, mov")
    # Sanitize filename
    ...
    try:
        ...
        logger.info(f"Video uploaded: {safe_name} ({len(contents)} bytes)")
        return AdminResponse(message=f"تم رفع الفيديو: {safe_name}")
    except Exception as e:
        logger.error(f"Video upload failed: {e}")
        return AdminResponse(status="error", message=f"فشل رفع الفيديو: {str(e)}")


@app.delete("/admin/videos/{filename}", response_model=AdminResponse)
async def admin_delete_video(filename: str) -> AdminResponse:
    """Delete an uploaded video file."""
    # Security: no path traversal
    safe_name = re.sub(r"[^\w\-.]", "_", filename)
    target = VIDEOS_DIR / safe_name
    if not target.exists():
        return AdminResponse(status="error", message="الملف غير موجود")
    target.unlink()
    return AdminResponse(message=f"تم حذف الفيديو: {safe_name}")


@app.get("/admin/current-video")
async def admin_current_video(since: float = 0.0) -> JSONResponse:
    """Poll endpoint: returns pending video command if newer than `since`."""
    # Flush any trigger fired from transcript analysis
    if _pending_video_trigger.get("trigger"):
        t = _pending_video_trigger.pop("trigger")
        video_url = f"/static/videos/{t['video_filename']}"
        video_command_state.set(video_url, t["trigger_id"])

    cmd = video_command_state.get_pending(since)
    return JSONResponse(content={"command": cmd})


@app.post("/admin/check-video-trigger", response_model=AdminResponse)
async def admin_check_video_trigger(request: SpeakRequest) -> AdminResponse:
    """Check if a transcript matches any video trigger keyword and fire it."""
    ...
    for trigger in triggers:
        for keyword in trigger.get("keywords", []):
            if keyword.strip().lower() in text:
                video_url = f"/static/videos/{trigger['video_filename']}"
                video_command_state.set(video_url, trigger["trigger_id"])
                logger.info(f"Video trigger matched: {trigger['trigger_id']} (keyword: {keyword})")
                return AdminResponse(message=f"تم تشغيل الفيديو: {trigger['label']}")
    return AdminResponse(message="لا يوجد تطابق")
